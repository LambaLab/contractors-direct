...
import { getScopeItemById, validateScopeRemoval } from '@/lib/scope/dependencies'
...
export default function ScopeCard({ scopeId, status, detectedScope, onToggle, summary }: Props) {
  ...
  const item = getScopeItemById(scopeId)
  if (!item) return null
            ...
            {status === 'confirmed' && (() => {
              ...
              return canRemove ? (
                ...
                <p className="mt-2 text-[10px] text-[var(--ov-text-muted,#727272)]/40">
                  Required by: {blockedBy.map(id => getScopeItemById(id)?.name ?? id).join(', ')}
                </p>
              )
            })()}
          </div>
